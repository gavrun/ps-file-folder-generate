...
$rootPath = Join-Path (Get-Location) 'root31'  # Root path of the SMB share "C:\path\to\smb\share" and SMB share "C:\path\to\smb\share\root\*"
...
# Initialize a single instance of the Random class
...
for ($folderIndex = 1; $folderIndex -le $folderCount; $folderIndex++) {
    ...
    for ($subfolderIndex = 1; $subfolderIndex -le $subfolderCount; $subfolderIndex++) {
        ...
        foreach ($fileName in $fileNamingPattern) {
            ...
            # Define a file size of $fileSizeMB in MB
            $fileSizeBytes = $fileSizeMB * 1024 * 1024  # File size in bytes

            # Initialize a byte array of the required size
            $byteArray = New-Object Byte[] $fileSizeBytes

            # Fill the byte array with random ASCII characters (printable range 32 to 126)
            for ($i = 0; $i -lt $fileSizeBytes; $i++) {
                $byteArray[$i] = $random.Next(32, 127)  # Get a random ASCII character
            }
